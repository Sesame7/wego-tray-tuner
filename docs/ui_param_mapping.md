...
This file maps wego-tray-tuner UI labels to the current upstream detector keys.

Reference implementation:

- `ui/control_panel.py` (UI labels and binding)
- `detect/wego_tray.py` (actual parameter consumption)
- `config/wego_tray/*.yaml` (default values and structure)
...
| Row `r` white threshold (Smax,Vmin) | `junction_line.white_by_row.row_index.s_max`, `junction_line.white_by_row.row_index.v_min` | csv2 int | Row r white mask (dynamic by row count) |
